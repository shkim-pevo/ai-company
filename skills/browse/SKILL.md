...
```bash
B=$(browse/bin/find-browse 2>/dev/null || skills/browse/browse/bin/find-browse 2>/dev/null || ~/.claude/skills/browse/browse/bin/find-browse 2>/dev/null)
if [ -n "$B" ]; then
  echo "READY: $B"
else
  echo "NEEDS_SETUP"
...
**How it works:**
1. Find the browse binary (see Setup above)
2. Run `cookie-import-browser` to detect installed browsers and open the picker UI
3. User selects which cookie domains to import in their browser
4. Cookies are decrypted and loaded into the Playwright session

### Import via interactive picker

```bash
$B cookie-import-browser
```

Auto-detects installed Chromium browsers (Comet, Chrome, Arc, Brave, Edge) and opens an interactive picker UI where you can:
...
Replace `comet` with the appropriate browser if specified by the user.
...
- Only domain names and cookie counts shown in picker UI — no cookie values exposed
- Imported cookies persist between commands, so they work immediately
